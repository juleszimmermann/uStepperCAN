...
#define STEPPRMM 200/1.25//full step pr. mm for the rail used in the demo
#define MMPRSTEP 1/(STEPPRMM*256)//mm pr. microstep
#define MMPRDEG MMPRSTEP*RES//mm pr. degree
#define STALLSENSITIVITY 5//sensitivity of the stall detection, between -64 and 63 - higher number is less sensitive
...
bool SetHome()
{
  ...
  stepper.moveToEnd(CCW, 200, STALLSENSITIVITY); // Set the current position as home
...
void loop()
{
  ...
  else
  {
    ...
    Angle_Demande = Angle_Demande*1.25*200;
    ...
    //{
      ...
      stepper.moveToAngle(Angle_Demande); // (ratio_reducteur * STEPSPERREV * (Angle_Demande/2)
      
    //}
    //Serial.println("Angle: " + String(Angle_Demande));
  }

  Serial.println("Angle demandé : " + String(Angle_Demande) + " | Home_Demande : " + String(Home_Demande) + " | Verin_Demande : " + String(Verin_Demande) + " | Angle_Reel : " + String(Angle_Reel));
  delay(10);
}